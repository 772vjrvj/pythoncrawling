...
class ReservationService:
    ...
    def register(self, req_json=None, resp_json=None):
        for entity in PayloadBuilder.extract_entities(resp_json):
            ...
            dom_data_list = DomState.get_all()

            if dom_data_list:
                for i in reversed(range(len(dom_data_list))):
                    dom_item = dom_data_list[i]

                    if (
                            dom_item.get("name") == payload.get("name")
                            and dom_item.get("phone") == payload.get("phone")
                            and dom_item.get("type") == "register"
                    ):
                        merged = dom_item.copy()

                        skip_keys = {"machineNumber", "name", "phone", "timestamp"}
                        for key, value in payload.items():
                            if key not in skip_keys:
                                merged[key] = value

                        # ✅ DomState 업데이트
                        DomState.update(i, merged)

                        # ✅ roomId만 병합된 값으로 교체
                        payload["roomId"] = merged.get("roomId")
