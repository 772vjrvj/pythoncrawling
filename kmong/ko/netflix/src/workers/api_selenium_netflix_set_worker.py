...
class ApiNetflixSetLoadWorker(QThread):
    ...
    def __init__(self, url_list, parent=None):
        ...
        self.baseUrl = "https://www.netflix.com"
        self.result_list = []
        ...
        if len(self.url_list) > 0:
            ...
            if login:
                ...
                for idx, url in enumerate(self.url_list, start=1):
                    ...
                    soup = self._fetch_place_info(url, result)
                    error_chk = self._error_chk(result)
                    if soup and not error_chk:
                        self._extract_netflix_data(soup, result)
                        ...
                        if 'watch' in url:
                            if not error_chk:
                                meta_data = self._get_episode_data(url, result)

                                if meta_data and (episode_data := self._get_episode_json(meta_data)):
    ...
    # 에피소드 json 가져오기
    def _get_episode_json(self, meta_data):
    ...
    # 에피소드 data 가져오기
    def _get_episode_data(self, url, result):
    ...
    # data 추출
    def _extract_netflix_data(self, soup, result):
        """
