...
def get_video_date_and_views_from_url(url, headers):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # ytInitialData 스크립트를 포함한 텍스트 찾기
            script_tag = soup.find('script', string=re.compile(r'var ytInitialData'))
            if script_tag:
                # ytInitialData 추출
                yt_initial_data_str = re.search(r'var ytInitialData\s*=\s*({.*?});', script_tag.string, re.DOTALL)
...
def extract_dates_from_urls(df, headers):
...
def save_dates_and_views_to_excel(data_list, output_file):
    """추출된 날짜와 조회수 리스트를 엑셀 파일로 저장"""
    # DataFrame 생성
    output_df = pd.DataFrame(data_list, columns=['날짜', '조회수'])
    # 엑셀 파일로 저장
    output_df.to_excel(output_file, index=False, encoding='utf-8-sig')
    print(f"엑셀 파일로 결과가 저장되었습니다: {output_file}")


def main():
    """메인 함수에서 엑셀 파일을 읽고, URL에서 날짜를 추출하여 엑셀로 저장"""
    file_path = '조승현2.xlsx'  # 엑셀 파일 경로를 입력하세요.
    output_file = '조승현2결과.xlsx'   # 결과를 저장할 엑셀 파일 이름
    ...
    headers = {
        ...
        "accept-encoding": "gzip, deflate, zstd",  # br 제거
    ...
    # URL에서 날짜 추출
    date_list = extract_dates_from_urls(df, headers)

    # 날짜 리스트를 엑셀로 저장
    save_dates_and_views_to_excel(date_list, output_file)
