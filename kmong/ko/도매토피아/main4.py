...
def fetch_goods_values(page, search_text):
    ...
    for i in range(1, page + 1):
        ...
        # 1. dd 태그의 class가 goodsDisplayCode 인 것 안에서
        dd_tags = soup.find_all('dd', class_='goodsDisplayCode')

        for dd in dd_tags:
            # label 태그의 class가 hand 인 것 찾기
            label_tag = dd.find('label', class_='hand')
            if label_tag:

                # span 태그의 class가 goods_scode 인 것의 text 출력
                span_tag = label_tag.find('span', class_='goods_scode')
                if span_tag:
                    product_code = span_tag.get_text(strip=True)
                    if product_code[:2] != search_text or "GKM" in product_code[:3] or "GKD" in product_code[:3]:
                        print(f"not product_code : {product_code}")
                        continue

                input_tag = label_tag.find('input', {'type': 'checkbox', 'class': 'list_goods_chk', 'name': 'goods_seq[]'})
