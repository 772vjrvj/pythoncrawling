...
from src.workers.api_oliveyoung_set_worker import ApiOliveyoungSetLoadWorker
...
class MainWindow(QWidget):
    ...
    def start_on_demand_worker(self):
        ...
        if self.collect_button.text() == "시작":
            ...
            if self.on_demand_worker is None:  # worker가 없다면 새로 생성
                if self.site == '올리브영':
                    self.on_demand_worker = ApiOliveyoungSetLoadWorker(self.url_list)
    ...
    def update_list(self, url_list):
        ...
        self.add_log(f'URL 목록 갯수 : {len(self.url_list)}')
        for url in url_list:
            self.add_log(url)
