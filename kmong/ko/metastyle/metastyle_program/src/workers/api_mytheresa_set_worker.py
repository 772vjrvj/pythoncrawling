    ...
    def total_cnt_cal(self):
        ...
        for index, item in enumerate(self.check_list, start=1):
            ...
            if isinstance(response_json, dict):
                data = response_json.get('data', {}).get('xProductListingPage', {})
                pagination = data.get('pagination', {})
                totalItems = pagination.get('totalItems')

                if totalItems:
                    total_cnt += int(totalItems)
                else:
                    self.log_signal.emit(f"{item}은 상품이 존재하지 않습니다.")
            else:
                self.log_signal.emit(f"{item}은 유효하지 않은 응답입니다.")
