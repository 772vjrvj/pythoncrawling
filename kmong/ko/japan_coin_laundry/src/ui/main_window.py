...
class MainWindow(QWidget):
    ...
    def set_layout(self):
        ...
        # 브랜드 URL 등록
        self.item_list_button = QPushButton("키워드 등록")
        self.item_list_button.setStyleSheet("""
                    ...
                    padding: 10px;
                """)
        self.item_list_button.setFixedWidth(100)  # 고정된 너비
        self.item_list_button.setFixedHeight(40)  # 고정된 높이
        self.item_list_button.setCursor(Qt.PointingHandCursor)
        self.item_list_button.clicked.connect(self.open_all_register_popup)
        ...
        # 왼쪽 버튼 레이아웃
        left_button_layout.addWidget(self.item_list_button)
    ...
    def update_list(self, item_list):
        ...
        self.add_log(f'항목 갯수 : {len(self.item_list)}')
    ...
    def start_on_demand_worker(self):
        if self.collect_button.text() == "시작":
            ...
            if self.on_demand_worker is None:  # worker가 없다면 새로 생성
                ...
                if worker_class:
                    self.on_demand_worker = worker_class(self.site, self.item_list)
