...
# 숫자 범위와 괄호로 감싸진 연도 패턴을 확인하는 함수
def is_page_range_year(line):
    return re.search(r'\d{1,4}\s*-\s*\d{1,4}\s*\(\d{4}\)', line) is not None
...
def extract_paper_title(text):
    ...
    exclude_keywords = [
        ...
        , 'PITUITARY–GONADAL', 'PHYTOTHERAPY RESEARCH', 'SHORT COMMUNICATION', 'Copyright', 'Ltd.', 'Article history', 'SCIENCE & TECHNOLOGY'
    ]
    ...
    for line in lines:
        ...
        if is_month_to_month_year(line_clean) or \
                is_four_digits_with_dot(line_clean) or \
                is_page_range_year(line_clean):
            continue
