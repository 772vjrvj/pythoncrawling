...
def handle_response(response: requests.Response, method_name: str):
    try:
        ...
        msg = f"[proxy_server] [api] : {method_name} 판도서버 {response.status_code} : 성공"
        log_info(msg)

        if not response.content or response.text.strip() == "":
            log_info(f"[proxy_server] [api] : {method_name} 응답 본문 없음 (빈 응답)")
            return None
...
def post(token: str, store_id: str, data: dict, param_type: str = None):
    ...
    log_info(f"[proxy_server] [api] : [POST] header : {headers}")
    log_info(f"[proxy_server] [api] : [POST] {url}\n{json.dumps(data, ensure_ascii=False, indent=2)}")
    try:
...
def put(token: str, store_id: str, data: dict, param_type: str = None):
    ...
    log_info(f"[proxy_server] [api] : [PUT] header : {headers}")
    log_info(f"[proxy_server] [api] : [PUT] {url}\n{json.dumps(data, ensure_ascii=False, indent=2)}")
    try:
        res = requests.put(url, json=data, headers=headers, proxies={"http": None, "https": None})
        return handle_response(res, 'PUT')
    except Exception as e:
        log_error(f"[proxy_server] [api] : PUT 요청 중 예외 발생: {e}")
        return None
...
def patch(token: str, store_id: str, data: dict, param_type: str = None):
    ...
    log_info(f"[proxy_server] [api] : [PATCH] header : {headers}")
    log_info(f"[proxy_server] [api] : [PATCH] {url}\n{json.dumps(data, ensure_ascii=False, indent=2)}")
    try:
        res = requests.patch(url, json=data, headers=headers, proxies={"http": None, "https": None})
        return handle_response(res, 'PATCH')
    except Exception as e:
        log_error(f"[proxy_server] [api] : PATCH 요청 중 예외 발생: {e}")
        return None
...
def local_web_req(token: str, store_id: str, data: dict, param_type: str = None):
    ...
    log_info(f"[proxy_server] [api] : [GET] header : {headers}")
    log_info(f"[proxy_server] [api] : [GET] {url}\n{json.dumps(data, ensure_ascii=False, indent=2)}")
    try:
...
def fetch_store_info(token: str, store_id: str):
    ...
    log_info(f"[proxy_server] [api] : 매장 정보 요청: {url}")
    try:
        res = requests.get(url, headers=headers, proxies={"http": None, "https": None})
        res.raise_for_status()
        return res.json()
    except requests.RequestException as err:
        msg = f"[proxy_server] [api] : 매장 정보 요청 실패: {err}"
        log_error(msg)
        return None
