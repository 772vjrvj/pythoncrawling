        ...
        try:
            ...
            for idx, logNo in enumerate(logNos):
                ...
                for ix, tag in enumerate(tags):
                    ...
                    new_print(f'아이디 : {blog_id}, 게시글 번호 : {logNo}, 태그 : {tags}, 검색글 수 : {len(search_logNos)}')
                    time_sleep()

                    for i, search_logNo in enumerate(search_logNos):
                        if str(search_logNo) == str(logNo):
                            new_print(f'아이디 : {blog_id}, 게시글 번호 : {logNo}, 태그 : {tags}, 찾은 검색글 위치 : {i+1} 번째')
                            hash_tag_cnt += 1
