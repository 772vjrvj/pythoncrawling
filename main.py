...
def extract_content_hash_tag(content_url):
    ...
    try:
        ...
        hashtags = [run["text"].replace("#", "") for run in runs if run["text"].strip() and "#" in run["text"]]
...
# 엑셀 파일로 저장하는 함수
def save_to_excel(content_data, file_name="YTN_KOREAN Youtube.xlsx"):
    df = pd.DataFrame(content_data)
    df.to_excel(file_name, index=False)

# 메인 함수
def main():
    file_path = "YTN_KOREAN Youtube.html"  # HTML 파일 경로 설정
