...
def fetch_search_results(query, page):
    try:
        ...
        new_print(f"플레이스 id 목록: {e}", "ERROR")
    return None
...
def main(query, total_queries, current_query_index, total_locs, locs_index, file_name):
    global login_server_check, final_all_ids, final_all_ids_list
    try:
        page = 1
        results = []
        all_ids = set()
        ...
        while True:
            ...
            # 현재 페이지에서 수집된 아이디에서 final_all_ids에 이미 있는 아이디를 제외한 후, all_ids에 추가
            new_ids = [place_id for place_id in ids_this_page if place_id not in final_all_ids]
            all_ids.update(new_ids)  # 중복되지 않는 아이디만 all_ids에 추가

            # final_all_ids에 새로운 아이디 추가 (중복 제거)
            final_all_ids.update(ids_this_page)
        ...
        new_print(f"main 크롤링 에러: {e}", "ERROR")
...
def login_to_server(username, password, session):
    ...
    try:
        ...
        messagebox.showerror("로그인", f"로그인에 실패하였습니다. {e}")
        return False
...
def change_password(session, username, current_password, new_password):
    ...
    try:
        ...
        messagebox.showerror("비밀변경", f"비밀변경에 실패하였습니다. {e}")
        return False
