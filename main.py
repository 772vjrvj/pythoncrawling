...
def get_video_date_from_url(url, headers):
...
def extract_dates_from_urls(df, headers):
    ...
    for index, url in enumerate(df['콘텐츠 주소']):
        if index == 5:
            break
        date = get_video_date_from_url(url, headers)
...
def main():
    ...
    output_file = '유튜브 결과 날짜.xlsx'   # 결과를 저장할 엑셀 파일 이름
    ...
    # 날짜 리스트를 엑셀로 저장
    save_dates_to_excel(date_list, output_file)
