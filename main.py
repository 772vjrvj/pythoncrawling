...
def email_setting(ws, index, email_data):
    ...
    for seller in email_data['판매처']:
        ...
        process_seller(index, row_object, seller, email_data, '판매처2', '상품명2', '가격2')
        process_seller(index, row_object, seller, email_data, '판매처3', '상품명3', '가격3')
...
def process_seller(index, row, seller, email_data, seller_key, product_key, price_key):
    ...
    if seller in row[seller_key]:
        ...
        if net_profit_rate >= email_data['마진율']:
            ...
            # 엑셀 행 내용 생성
            email_content = f"{excel_row} / {row[product_key]} / {row['메모']} / {row[seller_key]} / {net_profit_rate}% / {email_url}"
...
if __name__ == "__main__":
    ...
    end_row = 70    # 실제 row수보다 작거나 같게 설정
