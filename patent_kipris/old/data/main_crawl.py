...
def search_patents(patent_list):
    ...
    try:
        ...
        for num, patent in enumerate(patent_list, start=1):
            print(f"🔍 검색 중: {num} - {patent.reg_no}")
...
def main():
    ...
    # 등록번호가 NaN이 아닌 경우만, 7자리 zero-padding
    patent_list = [
        PatentData(str(reg_no).zfill(7))
        for reg_no in df['등록번호']
        if pd.notna(reg_no)
    ]
