...
# 2. ADDR_JIBUN → 시도, 시군구, 읍면동 추출
def split_jibun_parts(addr):
    try:
        addr = str(addr).strip()

        # 숫자 앞까지만 추출 후 공백 기준 분할
        pre_number_part = re.split(r'\s+(?=\d)', addr)[0]
        parts = pre_number_part.split()

        if len(parts) >= 4:  # ex: 전북 전주시 완산구 효자동3가
            sido = parts[0]
            sigungu = f"{parts[1]} {parts[2]}"
            eupmyun = parts[3]
        elif len(parts) == 3:  # ex: 대구 수성구 황금동
            sido = parts[0]
            sigungu = parts[1]
            eupmyun = parts[2]
        else:
        ...
        return pd.Series([sido, sigungu, eupmyun])
    except:
        return pd.Series(['', '', ''])

# 3. LOC_ALL 매칭
def match_region(sido, sigungu, eupmyun):
    match = loc_df[
        (loc_df['시도'] == sido) &
        (loc_df['시군구'] == sigungu) &
        (loc_df['읍면동'] == eupmyun)
    ...
    if not match.empty:
        row = match.iloc[0]
        return pd.Series([row['시도'], row['시군구'], row['읍면동']])
    else:
        return pd.Series(['', '', ''])
...
def process_nabiya(file_path, output_path):
    ...
    # ADDR_JIBUN → 시도, 시군구, 읍면동 분리
    df[['JIBUN_SIDO', 'JIBUN_SIGUNGU', 'JIBUN_EUPMYUN']] = df['ADDR_JIBUN'].apply(split_jibun_parts)

    # LOC_ALL 매핑
    df[['REGION_PROVINCE', 'REGION_CITY', 'REGION_TOWN']] = df.apply(
        lambda row: match_region(row['JIBUN_SIDO'], row['JIBUN_SIGUNGU'], row['JIBUN_EUPMYUN']),
        axis=1
    )
