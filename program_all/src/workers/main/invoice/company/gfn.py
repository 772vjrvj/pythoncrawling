# src/workers/main/invoice/company/gfn.py
import os, re
...
class GfnInvoiceParser:
    ...
    def __init__(self, folder="수입 선적서류/GFN", log_func=print):
    ...
    def parse_fields(self, page_text: str) -> dict:
        """
        Document Number / Document Date / Total Amount 전용 파서
        """
        ...
        m_doc = re.search(
            r'(?i)Document\s+Number\s+(\d+)\s+Document\s+Date\s+(\d{4}\.\d{2}\.\d{2})',
            page_text
        )
        if m_doc:
            res["Invoice no."] = m_doc.group(1).strip()
            raw_date = m_doc.group(2).strip()
        ...
        # 2) Total Amount … EUR
        m_amt = re.search(r'(?i)Total\s+Amount\s+([\d,]+\.\d{2})\s*EUR', page_text)
        if m_amt:
            num = m_amt.group(1)
            try:
                res["Amount"] = float(num.replace(",", ""))
            except ValueError:
                res["Amount"] = None
    ...
    def sync_excel(self, objs: list):
        ...
        # 헤더 위치 및 컬럼 인덱스
        header_row = 3
        headers = ["Order no.", "OC", "Invoice no.", "Amount", "Invoice date", "60 Due", "42 Amount", "42 Due", "Payment", "Note"]
        ...
        for order_no, obj in parsed_map.items():
            ...
            if isinstance(amount, (int, float)):
                ...
                # 회계 서식 + USD 심볼
                c_amount.number_format = '_-[$EUR]* #,##0.00_-;[Red]-[$USD]* #,##0.00_-;_-[$EUR]* "-"??_-;_-@_-'
            ...
            c_due = ws.cell(row=append_row, column=header_to_col["60 Due"])
            # row 번호에 맞춰 E열 참조
