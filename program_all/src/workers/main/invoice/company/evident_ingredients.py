# src/workers/main/invoice/company/evident_ingredients.py
import os, re
...
class EvidentInvoiceParser:
    ...
    def __init__(self, folder="수입 선적서류/Evident ingredients", log_func=print):
    ...
    def read_text(self, path):
        ...
        for i, p in enumerate(getattr(r, "pages", []), 1):
            ...
            if "INVOICE VKR" in (t.upper() if t else ""):
                result = t
        if result:
            return result
        else:
    ...
    # region 전용 파서 (Invoice VKR / Document Date / Total EUR)
    def parse_fields(self, page_text: str) -> dict:
        """
        Invoice VKR 문서 전용 필드 추출
        - Invoice no. : "Invoice VKRxxxxxx" → VKRxxxxxx
        - Invoice date : "Document Date" 다음 줄 (dd.mm.yyyy → yyyy-mm-dd)
        - Amount : "Total EUR" 다음 줄 (유럽식 1.234,56 → 1234.56)
        """
        ...
        # 1) Invoice no.
        for ln in lines:
            m = re.search(r'\bInvoice\s+(VKR[0-9]+)', ln, re.I)
            if m:
                res["Invoice no."] = m.group(1).strip()
                break

        # 2) Document Date → 다음 줄
        for idx, ln in enumerate(lines):
            if re.search(r'(?i)^Document\s+Date$', ln):
                if idx + 1 < len(lines):
                    raw_date = lines[idx + 1]
                    # 형식: dd.mm.yyyy → yyyy-mm-dd
                    m_dt = re.match(r'(\d{1,2})\.(\d{1,2})\.(\d{4})', raw_date)
                    if m_dt:
        ...
        # 3) Total EUR → 다음 줄
        for idx, ln in enumerate(lines):
            if re.search(r'(?i)^Total\s+EUR$', ln):
                if idx + 1 < len(lines):
                    raw_amt = lines[idx + 1]
                    # 유럽식 숫자: 4.737,50 → 4737.50
                    cleaned = raw_amt.replace(".", "").replace(",", ".")
                    try:
