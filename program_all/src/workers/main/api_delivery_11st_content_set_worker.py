...
class ApiDelivery11stContentSetLoadWorker(BaseApiWorker):
    ...
    def __init__(self):
        ...
        self.site_name = "11번가 주문 송장번호 택배사"
        self.excel_filename = ""
        self.data_obj_list = []
    ...
    def main(self):
        try:
            ...
            # === 신규: 원본 엑셀 순서 보존용 인덱스 추가 ===
            for idx, row in enumerate(self.excel_data_list):
                row["_orig_idx"] = idx   # 원래 엑셀의 순서
            ...
            # === 신규: 최종 결과를 원본 엑셀 순서대로 되돌리기 ===
            # _orig_idx 기준 오름차순 정렬
